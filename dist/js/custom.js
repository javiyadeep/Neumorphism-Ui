$(document).ready(function () {
  $(".code-btn1").click(function () {
    $(".code-view1").toggle(600);
    $(".code-view1").css("display", "block");
  });

  $(".code-btn2").click(function () {
    $(".code-view2").toggle(600);
    $(".code-view2").css("display", "block");
  });

  $(".code-btn3").click(function () {
    $(".code-view3").toggle(600);
    $(".code-view3").css("display", "block");
  });

  $(".code-btn4").click(function () {
    $(".code-view4").toggle(600);
    $(".code-view4").css("display", "block");
  });

  $(".code-btn5").click(function () {
    $(".code-view5").toggle(600);
    $(".code-view5").css("display", "block");
  });
});